...
protected:
    void onInit() override {
        ...
        m_fogShader = new Shader("assets/shaders/fog.vert", "assets/shaders/fog.frag");
    ...
    void onUpdate(float deltaTime) override {
        ...
        //     fKeyPressed = true;
        //     std::cout << "Fog effect: " << (m_enableFog ? "ON" : "OFF") << std::endl;
        // }
    ...
    void onRender() override {
        ...
        // ========== 第二阶段：应用雾效后处理 ==========
        if (m_enableFog && m_fbo && m_fogShader && m_quadVAO) {
            // 恢复默认framebuffer
            ...
            m_fogShader->setInt("sceneTexture", 0);
            ...
            // 相机参数
            m_fogShader->setFloat("nearPlane", 0.1f);
            m_fogShader->setFloat("farPlane", 1000.0f);
            m_fogShader->setMat4("invProjection", glm::inverse(m_camera->getProjectionMatrix()));
            m_fogShader->setMat4("invView", glm::inverse(m_camera->getViewMatrix()));
            
            // 雾效参数
            m_fogShader->setVec3("fogColor", m_fogColor);
            m_fogShader->setFloat("fogDensity", m_fogDensity);
            m_fogShader->setFloat("fogStart", m_fogStart);
            m_fogShader->setFloat("fogEnd", m_fogEnd);
            
            // 色调参数
            m_fogShader->setVec3("colorTint", m_colorTint);
            m_fogShader->setFloat("saturation", m_saturation);
            m_fogShader->setFloat("contrast", m_contrast);
            
            // // 雨效参数
            // m_fogShader->setFloat("rainIntensity", m_rainIntensity);
            // m_fogShader->setFloat("rainSpeed", m_rainSpeed);
            // m_fogShader->setFloat("time", static_cast<float>(glfwGetTime()));
...
private:
    ...
    // 烟雨滤镜参数（江南百景图风格：暖黄绿色调）
    glm::vec3 m_fogColor = glm::vec3(0.78f, 0.80f, 0.70f);  // 暖黄绿雾色
    float m_fogDensity = 0.5f;     // 雾密度
    float m_fogStart = 8.0f;       // 雾开始距离
    float m_fogEnd = 55.0f;        // 雾结束距离
    glm::vec3 m_colorTint = glm::vec3(0.95f, 0.92f, 0.80f);  // 暖黄色调（古画泛黄）
    float m_saturation = 0.70f;    // 降低饱和度（水墨感）
    float m_contrast = 0.88f;      // 柔和对比度
    bool m_enableFog = true;       // 是否启用雾效
    
    // 雨效参数
    float m_rainIntensity = 0.5f;  // 雨强度 (0-1)
    float m_rainSpeed = 1.5f;      // 雨速度
    ...
    void createFogPostProcessing() {
        ...
        std::cout << "Fog post-processing system created successfully." << std::endl;
    }
